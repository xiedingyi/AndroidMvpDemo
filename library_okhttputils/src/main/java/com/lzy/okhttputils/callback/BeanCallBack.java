...
public abstract class BeanCallBack<T> extends AbsCallback<T> {
    ...
    public T parseNetworkResponse(Response response) throws Exception {
        ...
        if (type instanceof ParameterizedType) {
            ...
            //Class<T> beanType =(Class) parameterizedType.getActualTypeArguments()[0];
            if(beanType == String.class) {
                //如果是String类型，直接返回字符串
                return (T) response.body().string();
            } else {

               Type beanTypes = new TypeToken<T>(){}.getType();
//             Gson gson = new Gson();
//             ApiResponse<List<Version>> av = gson.fromJson(versionInfo, beanType);
                ...
                return new Gson().fromJson(response.body().string(),beanTypes);
            }
        } else {
